...
@api_handler()
def handler(
    event: AppleWebhookEvent, context: LambdaContext
) -> WebhookResponse:
    """
    ...
    try:
        ...
        decoded_payload = subscription_service.verify_and_decode_apple_webhook(signed_payload)
...
def _process_notification(decoded_payload) -> None:
    ...
    try:
        ...
        if not transaction_id:
            ...
            if any(no_transaction_type in notification_type for no_transaction_type in [
        ...
        if 'DID_RENEW' in notification_type:
            ...
            assert user_id is not None, f"User ID should exist after successful cancellation for transaction {transaction_id}"
            ...
            user_id = subscription_service.find_user_id_by_transaction_id(transaction_id)
            assert user_id is not None, f"User ID should exist after successful payment failure handling for transaction {transaction_id}"
            user_service.downgrade_user_tier(user_id, UserTier.FREE)
        ...
        elif 'DID_CHANGE_RENEWAL_STATUS' in notification_type or 'DID_CHANGE_RENEWAL_PREF' in notification_type:
            # Status/preference changes - log but don't fail
            logger.log_business_event(
                "renewal_change_processed",
                {"notification_type": notification_type}
            )
...
def _extract_transaction_id_from_data(data) -> Optional[str]:
    ...
    try:
        ...
        if hasattr(data, 'signed_transaction_info') and data.signed_transaction_info:
            ...
            verifier = subscription_service.apple_storekit_service._get_signed_data_verifier(StoreEnvironment.PRODUCTION)

            # Decode the signed transaction to get the transaction ID
            decoded_transaction = verifier.verify_and_decode_signed_transaction(data.signed_transaction_info)

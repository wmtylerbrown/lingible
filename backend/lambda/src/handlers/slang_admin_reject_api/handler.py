"""Lambda handler for admin rejection of slang submissions."""
...
def handler(event: PathParameterEvent, context: LambdaContext) -> AdminApprovalResponse:
    """Handle admin rejection of slang submissions (admin only)."""
    ...
    # For now, any authenticated user can reject (will add proper auth in CDK)

    # Reject the submission
    response = submission_service.admin_reject(submission_id, admin_user_id)

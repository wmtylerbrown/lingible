...
class AppleStoreKitService:
    ...
    def _get_signed_data_verifier(self, environment: StoreEnvironment) -> SignedDataVerifier:
    ...
    def validate_transaction(self, request: ReceiptValidationRequest) -> ReceiptValidationResult:
        """
        ...
        try:
            if request.transaction_data.provider != SubscriptionProvider.APPLE:
                raise ValidationError(f"Apple SDK only supports Apple transactions, got: {request.transaction_data.provider}")
            ...
            validated_data = self._create_validated_transaction_data(transaction_response, request.transaction_data)
    ...
    def _get_transaction_info(self, transaction_id: str, environment: StoreEnvironment) -> Optional[TransactionInfoResponse]:
        """
    ...
    def verify_and_decode_webhook(self, signed_payload: str, environment: StoreEnvironment) -> Optional[ResponseBodyV2DecodedPayload]:
        """
    ...
    def verify_webhook_signature(self, signed_payload: str, environment: StoreEnvironment = StoreEnvironment.PRODUCTION) -> bool:
        """
        ...
        """
        try:
            decoded_payload = self.verify_and_decode_webhook(signed_payload, environment)
    ...
    def _extract_transaction_data(self, signed_transaction: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _create_validated_transaction_data(
        self,
        apple_response: TransactionInfoResponse,
    ...
    def _load_apple_root_certificates(self) -> List[bytes]:
        ...
        apple_root_ca_g3 = bytes([
            0x30, 0x82, 0x02, 0x43, 0x30, 0x82, 0x01, 0xAB, 0xA0, 0x03, 0x02, 0x01, 0x02, 0x02, 0x10, 0x15,
            0x17, 0x8B, 0x5F, 0x9A, 0x5C, 0x50, 0x2E, 0x8F, 0x10, 0x2C, 0xFB, 0xCA, 0x7D, 0x86, 0x30, 0x0D,
            0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x0B, 0x05, 0x00, 0x30, 0x6C, 0x31,
            ...
            0x74, 0x68, 0x6F, 0x72, 0x69, 0x74, 0x79, 0x31, 0x20, 0x30, 0x1E, 0x06, 0x03, 0x55, 0x04, 0x03,
            0x13, 0x17, 0x41, 0x70, 0x70, 0x6C, 0x65, 0x20, 0x52, 0x6F, 0x6F, 0x74, 0x20, 0x43, 0x41, 0x20,
            0x2D, 0x20, 0x47, 0x33, 0x30, 0x1E, 0x17, 0x0D, 0x31, 0x37, 0x30, 0x35, 0x31, 0x30, 0x30, 0x30,
            0x30, 0x30, 0x30, 0x30, 0x5A, 0x17, 0x0D, 0x33, 0x30, 0x30, 0x35, 0x31, 0x30, 0x30, 0x30, 0x30,
            0x30, 0x30, 0x30, 0x5A, 0x30, 0x6C, 0x31, 0x0B, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13,
            ...
            ,
            ...
            0x1D, 0x0E, 0x04, 0x16, 0x04, 0x14, 0x63, 0x34, 0x3A, 0xBF, 0xB8, 0x9A, 0x6A, 0x03, 0xEB, 0xB5,
            0x7E, 0x9B, 0x3F, 0x5F, 0xA7, 0xBE, 0x7C, 0x4F, 0x5C, 0x75, 0x6F, 0x30, 0x17, 0xB3, 0xA8, 0xC4,
            0x88, 0xC3, 0x65, 0x3E, 0x91, 0x79
        ])
